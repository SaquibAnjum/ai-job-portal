...
const CandidateDashboard = () => {
  ...
  const getStrengthBadge = (pct) => {
    if (pct >= 90) return { label: 'All-Star Profile', color: 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' };
    if (pct >= 70) return { label: 'Strong Profile', color: 'text-indigo-400 border-indigo-500/30 bg-indigo-500/10' };
    if (pct >= 40) return { label: 'Intermediate Profile', color: 'text-amber-400 border-amber-500/30 bg-amber-500/10' };
    return { label: 'Incomplete Profile', color: 'text-rose-400 border-rose-500/30 bg-rose-500/10' };
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-xs font-bold text-slate-400">Loading Smart Dashboard & AI Analytics...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full space-y-8">
        ...
        <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative overflow-hidden">
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-xl font-extrabold text-white">{user?.name || 'Candidate'}</h1>
                <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${strengthBadge.color}`}>
                  {strengthBadge.label}
                </span>
                {user?.isVerified && (
                  <span className="flex items-center gap-1 text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-md border border-emerald-500/30">
                    <ShieldCheck className="w-3 h-3" /> Verified Candidate
                  </span>
                )}
              </div>
              <p className="text-xs text-indigo-400 font-semibold">{profile?.headline || 'Full Stack Software Engineer'}</p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => setIsEditProfileOpen(true)}
              className="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-white text-xs font-bold rounded-xl transition flex items-center gap-1.5 shadow-md"
            >
            ...
            {profile?.resumeUrl && (
              <button
                ...
                className="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-white text-xs font-bold rounded-xl transition flex items-center gap-1.5 shadow-md"
              >
                <Eye className="w-4 h-4 text-purple-400" /> Preview Resume
              </button>
            )}
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Profile Completion</span>
            <div className="flex items-baseline justify-between">
              <span className="text-xl font-extrabold text-indigo-400">{completionPct}%</span>
          ...
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-2">
            ...
            <div className="flex items-baseline justify-between">
              <span className="text-xl font-extrabold text-emerald-400">{profile?.atsReport?.atsScore || 86}/100</span>
              <span className="text-[10px] text-emerald-400 font-bold">Passing</span>
            </div>
            <div className="w-full h-1.5 bg-slate-900 rounded-full overflow-hidden">
          ...
          {/* Average Job Match */}
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Avg Job Match %</span>
            <div className="flex items-baseline justify-between">
              <span className="text-xl font-extrabold text-purple-400">{avgMatchScore}%</span>
          ...
          {/* Recommended Jobs */}
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Recommended Jobs</span>
            <div className="flex items-baseline justify-between">
              <span className="text-xl font-extrabold text-teal-400">{recommendations.length}</span>
              <span className="text-[10px] text-teal-400 font-bold">Matched</span>
            </div>
            <p className="text-[10px] text-slate-400 truncate">Updated by Gemini</p>
          </div>

          {/* Resume Version */}
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Cloudinary Resume</span>
            <div className="flex items-baseline justify-between">
            ...
            <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-4">
              <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-indigo-400" /> AI Skill Insights
                </span>
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-[11px] text-slate-400 mb-2 font-semibold">Extracted Profile Skills:</p>
                  <div className="flex flex-wrap gap-1.5">
                    {profile?.skills?.map((s, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded-lg bg-indigo-500/10 text-indigo-300 text-[11px] font-medium border border-indigo-500/20">
                ...
                {aggregatedMissingSkills.length > 0 && (
                  <div className="pt-3 border-t border-slate-800">
                    <p className="text-[11px] text-amber-400 mb-2 font-semibold flex items-center gap-1">
                      <AlertCircle className="w-3.5 h-3.5" /> High-Demand Missing Skills for Jobs:
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {aggregatedMissingSkills.map((sk, idx) => (
                        <span key={idx} className="px-2.5 py-1 rounded-lg bg-amber-950/40 text-amber-300 text-[11px] font-medium border border-amber-800/30">
                          + {sk}
                        </span>
                      ))}
                    </div>
                ...
                <div className="pt-3 border-t border-slate-800">
                  <p className="text-[11px] text-slate-400 mb-1.5 font-semibold flex items-center gap-1">
                    <BookOpen className="w-3.5 h-3.5 text-purple-400" /> Gemini Executive Summary:
                  </p>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-base font-bold text-white flex items-center gap-2">
                  <Clock className="w-4 h-4 text-indigo-400" /> Application Tracker ({applications.length})
                </h2>
              </div>

              {applications.length === 0 ? (
                <p className="text-xs text-slate-400">You haven't submitted any job applications yet.</p>
              ) : (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <img
                          ...
                          className="w-10 h-10 rounded-xl object-cover bg-slate-900 border border-slate-800"
                        />
                        <div>
                          <h4 className="text-xs font-bold text-white">{app.job?.title}</h4>
                          <p className="text-[11px] text-slate-400">{app.job?.company?.name}</p>
                          {app.rejectionReason && (
                            <p className="text-[10px] text-rose-400 mt-1">Feedback: {app.rejectionReason}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {app.status !== 'Rejected' && app.status !== 'Offered' && (
                          <button
                            ...
                            className="px-2.5 py-1 bg-slate-900 hover:bg-rose-950 text-slate-400 hover:text-rose-300 border border-slate-800 text-[10px] font-semibold rounded-lg transition"
                          >
                            Withdraw
                          </button>
                        )}
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-bold text-white flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-indigo-400" /> AI Recommended Positions for You
                </h2>
                <span className="text-xs text-slate-400">Matched with your Gemini Resume Profile</span>
              </div>

  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col justify-between transition-colors duration-300">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white mb-6 transition-colors"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Listings
        </button>

        <div className="glass-card p-6 sm:p-10 rounded-3xl border border-slate-200/80 dark:border-slate-800 space-y-8">
          {/* Header Info */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-4">
              <img
                ...
                className="w-16 h-16 rounded-2xl object-cover ring-2 ring-indigo-500/30 bg-slate-100 dark:bg-slate-900"
              />
              <div>
                <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">{selectedJob.title}</h1>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-400">{selectedJob.company?.name}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="p-3 bg-slate-100/80 dark:bg-slate-950/60 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-xs font-bold text-slate-800 dark:text-slate-200 mt-0.5 flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" /> {selectedJob.location}
              </p>
            </div>
            ...
            <div className="p-3 bg-slate-100/80 dark:bg-slate-950/60 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-xs font-bold text-slate-800 dark:text-slate-200 mt-0.5 flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> {selectedJob.experienceLevel}
              </p>
            </div>
          </div>
          ...
          {/* Required Skills */}
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Required Technical Skills</h3>
            <div className="flex flex-wrap gap-2">
              {selectedJob.requiredSkills?.map((skill, idx) => (
                <span key={idx} className="px-3 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 text-xs font-semibold border border-indigo-200 dark:border-indigo-500/30">
                  {skill}
                </span>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Job Overview & Responsibilities</h3>
            <div className="text-xs text-slate-700 dark:text-slate-300 whitespace-pre-line leading-relaxed space-y-2 bg-slate-100/60 dark:bg-slate-950/40 p-6 rounded-2xl border border-slate-200 dark:border-slate-800/80">
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
            {appliedSuccess ? (
              <div className="p-6 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-2xl text-center space-y-2">
                <CheckCircle2 className="w-8 h-8 text-emerald-600 dark:text-emerald-400 mx-auto" />
                <h4 className="text-sm font-bold text-emerald-800 dark:text-emerald-300">Application Submitted!</h4>
                <p className="text-xs text-slate-600 dark:text-slate-400">
                  Gemini AI analyzed your resume and calculated an <span className="font-bold text-slate-900 dark:text-white">{appliedSuccess.aiMatchAnalysis?.matchScore}% Match Score</span>.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Apply with AI Match Screening
                  </h3>
                  <button
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Cover Note (Optional)</label>
                  <textarea
                    rows={3}

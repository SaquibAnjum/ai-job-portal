      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-white flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-indigo-400" /> Upload & Parse Resume
          </h3>
      ...
      {parsedInfo && (
        <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl space-y-2">
          ...
          <p className="text-xs text-slate-300">
            Extracted <span className="font-bold text-white">{parsedInfo.skills?.length || 0} skills</span> and{' '}
            <span className="font-bold text-white">{parsedInfo.totalExperienceYears || 2} years of experience</span>.
          </p>
          <div className="flex flex-wrap gap-1.5 pt-2">
            {parsedInfo.skills?.slice(0, 8).map((skill, idx) => (
              <span key={idx} className="px-2 py-0.5 rounded-md bg-emerald-500/20 text-emerald-300 text-[10px] font-medium">
                {skill}
              </span>
            ))}
          </div>

...
function AICoverLetterModal({ isOpen, onClose, jobTitle, companyName, onApplyWithCoverLetter }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-2xl w-full p-6 shadow-2xl relative">
        <button
          onClick={onClose}
        ...
        <p className="text-slate-400 text-sm mb-4">
          Generate a tailored, professional cover letter for <strong className="text-indigo-400">{jobTitle}</strong> at <strong className="text-indigo-400">{companyName}</strong>.
        </p>

        {!coverLetter && (
          <div className="text-center py-8 border-2 border-dashed border-slate-700 rounded-xl mb-4">
            <p className="text-slate-400 text-sm mb-4">Let Gemini AI draft a personalized cover letter matching your resume skills.</p>
            <button
        ...
        {coverLetter && (
          <div className="mb-4">
            <textarea
              ...
              className="w-full p-4 bg-slate-900 border border-slate-700 rounded-xl text-slate-200 text-sm leading-relaxed outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-slate-200 text-sm font-semibold rounded-xl transition"
          >
            Cancel
          </button>
          {coverLetter && (

...
import { Bot, Send, X, Sparkles, User, Loader2, MessageSquare, Zap } from 'lucide-react';
...
const AIChatDrawer = () => {
  ...
  return (
    <>
      ...
      {/* Slide-over Drawer */}
      {isOpen && (
        <div className="fixed bottom-20 right-6 z-50 w-96 max-w-[calc(100vw-2rem)] h-[560px] glass-card rounded-3xl border border-slate-800 shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4">
          {/* Header */}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-emerald-400 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span> Powered by Gemini
                </p>
              </div>
            </div>
            <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-white p-1 rounded-lg transition">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-slate-950/70">
            {messages.map((msg, i) => (
              ...
              >
                {msg.sender === 'ai' && (
                  <div className="w-7 h-7 rounded-xl bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center shrink-0 shadow">
            ...
            {loading && (
              <div className="flex items-center gap-2 text-xs text-indigo-400 p-2 bg-slate-900/40 rounded-xl border border-slate-800/50 w-fit">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-slate-900 hover:bg-indigo-950 text-slate-300 hover:text-indigo-300 border border-slate-800 hover:border-indigo-500/40 rounded-lg text-[10px] font-medium transition"
                    >
                      💡 {chip}
                    </button>
                  ))}
          ...
          <form onSubmit={handleSend} className="p-3 bg-slate-900 border-t border-slate-800 flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <button
              type="submit"

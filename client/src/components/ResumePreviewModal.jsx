...
const ResumePreviewModal = ({ isOpen, onClose, resumeUrl, originalName }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-4xl w-full h-[85vh] flex flex-col shadow-2xl overflow-hidden relative">
        <div className="p-4 sm:p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">{originalName || 'Resume Preview'}</h3>
              <p className="text-[11px] text-slate-400">Stored & Parsed on Cloudinary / MongoDB</p>
            </div>
          </div>
        ...
        <div className="flex-1 bg-slate-950 p-4 flex items-center justify-center overflow-hidden">
          {resumeUrl ? (
            ...
            <div className="text-center space-y-3">
              ...
              <p className="text-xs text-slate-400">No active resume document uploaded yet.</p>
            </div>
          )}
        </div>
      </div>

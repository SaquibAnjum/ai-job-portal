...
import { Sparkles, Briefcase, User, LogOut, Bell, LayoutDashboard, MessageSquare, Calendar } from 'lucide-react';
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
          <Link
            ...
            className={`transition-colors flex items-center gap-1.5 ${
              location.pathname === '/jobs' ? 'text-indigo-400 font-semibold' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          {isAuthenticated && (
            <>
              <Link
                ...
                className={`transition-colors flex items-center gap-1.5 ${
                  location.pathname.includes('dashboard') ? 'text-indigo-400 font-semibold' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <Link
                to="/messages"
                className={`transition-colors flex items-center gap-1.5 ${
                  location.pathname === '/messages' ? 'text-indigo-400 font-semibold' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <Link
                to="/interviews"
                className={`transition-colors flex items-center gap-1.5 ${
                  location.pathname === '/interviews' ? 'text-indigo-400 font-semibold' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
          ...
          {isAuthenticated ? (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 p-1.5 rounded-xl border border-slate-800 hover:bg-slate-900 transition-colors"
              >
                <img
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-56 glass-card rounded-xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2 text-xs text-slate-300 hover:bg-indigo-600/20 hover:text-white"
                  >
                    <User className="w-4 h-4 text-indigo-400" /> Manage Profile
                  </Link>
                  <button
                    onClick={handleLogout}
                    className="w-full text-left flex items-center gap-2 px-4 py-2 text-xs text-rose-400 hover:bg-rose-500/10 hover:text-rose-300"
                  >
